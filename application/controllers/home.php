...
class home extends CI_Controller {
	public function index() {
		...
		// if user isn't logged in then redirect to the startpage
			...
			header("Location: index.php/routers/regUserDash?id=null&requestedPageType=startpage");
		}
		...
		$array_items = array('logged' => '', 'mychurchid' => '', 'userid' => '');
		...
		$this->db->query("INSERT IGNORE INTO users (username, password, firstname, lastname, email, account_type, userid, status, isLeader, defaultImgURI, isChurchRegistered) VALUES('{$username}', '{$hashedPass}', '{$firstname}', '{$lastname}', '{$email}', 'Regular Member', '{$idgen}', 'Pass', 'Nope', 'images/church.jpg', 'No')");
		$query = $this->db->query("SELECT * FROM users WHERE account_type LIKE '%Administrator%'");
			...
			// Make vars into sessions:
			$this->session->set_userdata('logged', '1');
			$this->session->set_userdata('userid', $idgen);
			$this->session->set_userdata("username", $username);
			
			redirect(base_url().'index.php/routers/updateProfileImgRegUserRegister');
			...
			if ($query->num_rows() > 0) {
				...
				if ($row->status == "Pass") {
					...
					if ($churchSelector->num_rows() > 0) {
		  			...
		  			$query6 = $this->db->query("SELECT * FROM churchmembers WHERE cMuserId = '{$userid}'"); // grab currently logged in users church
		  			foreach ($query6->result() as $row1) {
		  				$this->session->set_userdata('mychurchid', $row1->cMchurchId);
		  			}
					$this->session->set_userdata('logged', "1");
		...
		$query = $this->db->query("SELECT * FROM users WHERE username = '{$postedUser}' AND password = '{$hashedPass}'");
			if ($query->num_rows() > 0) {
				...
				if ($row->status == "Pass") {
					...
					header("Location: ".base_url()."index.php/routers/regUserDash?requestedPageType=wall_1&loggingStatus=loggedIn");
				} elseif ($row->status == "Fail" || $row->status == "Pending") {
					echo json_encode(array('session_statfse' => false));
				}
			} else {
				echo "sorry, but the login details you entered are incorrect. Please go <a href='javascript: history.go(-1)'>back</a> and try again or register if you do not have an account.";
			}
		} public function checkIsUsernameTaken() {
			$username = $this->input->post('username');
