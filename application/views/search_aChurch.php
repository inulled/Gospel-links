...
<?php
		$areacodes = array(
				...
				$row = $query->row();
				if ($query->num_rows() > 0) {
					// runs through array equaling out areacode field with areacode array
					foreach ($query->result() as $row) {
					foreach($areacodes as $key => $val) {
						if ($key == $row->areacode) { ?>
<div class="areaCodeList" id="areaCodeList"><?=$row->areacode." - ".$row->locational_state;?></br></div>
<div class="churchList font1" id="churchList" style="display: none"><?=$row->church_name.nl2br("\n").$row->locational_city.nl2br("\n").$row->locational_zip;?>
<br><a class="link-font1" target="_top" href="<?=base_url()?>index.php/routers/regUserDash?churchid=<?=$row->churchId?>&requestedPageType=wall_1">Access church profile</a>
</div>
<?php					}
					}
				}
