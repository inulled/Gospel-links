...
<script type="text/javascript">
	...
	function wallTable(defaultImgURI, firstname, lastname, entryData, entryCreationDateTime, idWallPosts) {
		return '<table id="load_status_out-' + idWallPosts + '" cellpadding="0" cellspacing="0" style="width: 376px; height: 63px" class="status-border-bottom-box1"><tr>'+
...
<table cellpadding="0" cellspacing="0" style="height: 282px">
	<!-- MSTableType="layout" -->
	<tr>
        ...
        foreach ($query->result() as $row) { ?>
				<img id="defaultImg" src="<?=base_url().$row->defaultImgURI?>" width="110" height="120" style="padding: 5pt" /><br />
		<?php } ?>
		<!-- MSCellType="empty" -->
		&nbsp;</td>
...
foreach ($query2->result() as $row) { ?>
    ...
    <tr>
        ...
        <a class="link-font2" href='<?=base_url()?>index.php/routers/regUserDash?id=<?=$row->userid?>&requestedPageType=userProfiles'>
		View Profile<br /></a>
	<?php // the following php is the bootstrapper for the wall
	if ($userid == $row->userid) { // if user is the owner of the wall post show delete button otherwise do nothing ?>
		<a class="delpost link-font2" delpost="<?=$row->idwallPosts?>" href="javascript:void(0)" id="delPost">
		Delete Post</a>
	<?php } ?>
        </td>
        <td id="del" valign="top" style="width: 458px; height: 7px;">
...
<?php } } else { ?>
	...
	<br>
	<div class="font2" style="text-align: center">Sorry but you&#39;re not 
		associated with any churches. Click <a class="link-font1 fancybox.iframe" href="<?=base_url()?>index.php/routers/search_aChurch" id="findChurch">
		here</a> to find your church.</div>
	<?php } ?>
		<div id="newData"></div>
</div>
</div>
</td></td>
		<td style="height: 243px; width: 2px"></td>
	</tr>
</table>
